...
import { mergeOrAddHotspot, addTransaction, getHotspots } from '../services/storage'; // Updated Import
import CustomDialog from './CustomDialog';

interface JournalEntryProps {
  currentTime: { dayName: string; timeString: string; fullDate: Date; };
  shiftState: ShiftState | null; // Strategy Awareness
  onSaved: () => void;
}
...
const JournalEntry: React.FC<JournalEntryProps> = ({ currentTime, shiftState, onSaved }) => {
  ...
  // --- SMART PARSING LOGIC (Sama seperti sebelumnya) ---
  ...
  const toggleVoice = () => {
      ...
      try {
          ...
          rec.onstart = () => { setIsListening(true); setTranscriptPreview("Mendengarkan..."); vibrate(20); if (voiceTimeoutRef.current) clearTimeout(voiceTimeoutRef.current); voiceTimeoutRef.current = setTimeout(() => { stopVoice(); setTranscriptPreview("Waktu habis."); }, 8000); };
          rec.onend = () => { stopVoice(); vibrate(20); };
          rec.onresult = (e: any) => { if (voiceTimeoutRef.current) clearTimeout(voiceTimeoutRef.current); voiceTimeoutRef.current = setTimeout(() => stopVoice(), 5000); const text = Array.from(e.results).map((result: any) => result[0].transcript).join(''); setTranscriptPreview(text); if (e.results[0].isFinal) { processSmartVoice(text); playSound('success'); } };
          recognitionRef.current = rec; rec.start(); playSound('click');
      } catch (e) { console.error(e); stopVoice(); }
  };
  const stopVoice = () => { if (recognitionRef.current) recognitionRef.current.stop(); if (voiceTimeoutRef.current) clearTimeout(voiceTimeoutRef.current); setIsListening(false); }

  const saveLogic = () => {
    const argoVal = parseCurrencyInput(argoRaw);
    if (!argoVal || isNaN(argoVal) || argoVal <= 0) { 
        setAlert({show: true, type:'alert', title:'Gagal', msg: "Nominal belum diisi!"}); return; 
    }
    setIsSubmitting(true); vibrate(50); playSound('success');
    ...
    // --- LEARNING LOOP: SAVE TO RADAR (SMART MERGING) ---
    // Jika ada koordinat, simpan sebagai Hotspot (User Entry)
    if (coords) {
        ...
        const newHotspot: Hotspot = {
            ...
            category, lat: coords.lat, lng: coords.lng, zone: 'User History',
            notes: `Riwayat Gacor Anda di jam segini.`, 
            isUserEntry: true, // INI KUNCINYA
            isDaily: false, 
            baseScore: 100,
            validations: [{ date: currentTime.fullDate.toISOString().split('T')[0], isAccurate: true }]
        };
        mergeOrAddHotspot(newHotspot); // Menggunakan Smart Merging
    }
    ...
    setAlert({ show: true, type: 'info', title: feedbackTitle, msg: feedbackMsg });
  }

  const handleAlertClose = () => {
      setAlert({show: false, type:'info', title:'', msg:''});
      if (isSubmitting) { setIsSubmitting(false); onSaved(); }
  }
  ...
  return (
    <div className="h-full flex flex-col bg-black pb-32">
      <CustomDialog isOpen={alert.show} type={alert.type} title={alert.title} message={alert.msg} onConfirm={handleAlertClose} />
      ...
      <form onSubmit={handleSubmit} className="flex-1 flex flex-col pt-6 px-5 space-y-6">
          ...
          <div className="bg-[#1a1a1a] rounded-3xl p-6 border border-gray-800 relative">
             <div className="flex justify-between items-start mb-2">
                 ...
                 <div className={`px-2 py-0.5 rounded text-[9px] font-bold flex items-center gap-1 ${coords ? 'text-emerald-500 bg-emerald-900/20' : 'text-red-500 bg-red-900/20'}`}>
                    <MapPin size={10} /> {coords ? 'DILOKASI' : 'NO GPS'}
                 </div>
             </div>
             <div className="flex items-center gap-2 my-2 relative">
                 <span className="text-2xl font-bold text-gray-600">Rp</span>
                 <input autoFocus required type="text" inputMode="numeric" placeholder="0" value={argoRaw} onChange={e => setArgoRaw(formatCurrencyInput(e.target.value))} className="w-full bg-transparent text-5xl font-mono font-black text-white focus:outline-none placeholder-gray-800" />
                  {argoRaw && (<button type="button" onClick={() => setArgoRaw('')} className="p-2 bg-gray-800 rounded-full text-gray-400 absolute right-0"><X size={16} /></button>)}
             </div>
             <div className="flex bg-black/50 p-1 rounded-xl mt-4 border border-gray-800/50">
                 <button type="button" onClick={() => setIsCash(true)} className={`flex-1 py-2 rounded-lg text-[10px] font-bold flex items-center justify-center gap-2 transition-all ${isCash ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}><Banknote size={14} /> Tunai</button>
                 <button type="button" onClick={() => setIsCash(false)} className={`flex-1 py-2 rounded-lg text-[10px] font-bold flex items-center justify-center gap-2 transition-all ${!isCash ? 'bg-purple-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}><CreditCard size={14} /> App</button>
             </div>
          </div>
          ...
          <div className="relative">
             ...
             <button type="button" onClick={toggleVoice} className={`w-full py-4 rounded-2xl border transition-all flex items-center justify-between px-5 group ${isListening ? 'bg-red-900/20 border-red-500 text-red-500' : 'bg-[#1a1a1a] border-gray-800 text-gray-400 hover:border-gray-600'}`}>
                 <div className="flex items-center gap-4">
                     <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isListening ? 'bg-red-600 text-white animate-bounce' : 'bg-black text-gray-500 group-hover:text-white'}`}><Mic size={20} /></div>
                     <div className="text-left"><span className="block text-[9px] uppercase font-bold tracking-wider opacity-70">Smart Voice</span><span className={`block font-bold text-sm ${isListening ? 'text-white' : 'text-gray-300'}`}>{isListening ? 'Mendengarkan...' : 'Tekan & Bicara'}</span></div>
                 </div>
                 {transcriptPreview && (<div className="text-right max-w-[100px]"><span className="text-[9px] text-emerald-400 font-mono italic truncate block">"{transcriptPreview}"</span></div>)}
             </button>
             {!isListening && !transcriptPreview && (<p className="text-[9px] text-gray-600 text-center mt-2">Contoh: "Gojek Bike 15 ribu tunai di Stasiun"</p>)}
          </div>
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                  {[{id: 'Bike', icon: <Bike size={18}/>, label: 'Bike'}, {id: 'Food', icon: <Utensils size={18}/>, label: 'Food'}, {id: 'Send', icon: <Package size={18}/>, label: 'Kirim'}, {id: 'Shop', icon: <ShoppingBag size={18}/>, label: 'Jajan'}].map(s => (
                      <button key={s.id} type="button" onClick={() => { setServiceType(s.id as any); vibrate(10); }} className={`py-3 rounded-2xl flex flex-col items-center gap-1 transition-all border ${serviceType === s.id ? 'bg-app-primary border-app-primary text-black' : 'bg-[#1a1a1a] border-gray-800 text-gray-500 hover:bg-gray-800'}`}>{s.icon} <span className="text-[10px] font-bold uppercase">{s.label}</span></button>
                  ))}
              </div>

              <div className="relative">
                   <input type="text" placeholder="Lokasi jemput (Opsional)..." value={origin} onChange={e => setOrigin(e.target.value)} className="w-full bg-[#1a1a1a] border border-gray-800 rounded-xl px-4 py-3 text-xs text-white focus:border-app-primary focus:outline-none placeholder-gray-600" />
          ...
          {/* 4. FOOTER ACTION */}
          <div className="pt-2">
              <button type="submit" disabled={isSubmitting} className={`w-full py-4 rounded-2xl font-black text-lg flex items-center justify-center gap-2 shadow-glow transition-all active:scale-[0.98] ${isSubmitting ? 'bg-gray-800 text-gray-500' : 'bg-app-primary text-black hover:bg-yellow-400'}`}>
